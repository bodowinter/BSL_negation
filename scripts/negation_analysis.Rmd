...
```{r, message = FALSE, warning = FALSE}
neg <- read_csv('../data/BSL_negation_12_June_2020.csv')
```
...
```{r}
neg <- mutate(neg,
              HandednessCode = ifelse(str_detect(CLU_value, 'LH'), 'LH', 'RH'),
              Handedness = ifelse(HandednessCode == 'LH', 'left-handed', 'right-handed'))
```
...
```{r}
neg <- mutate(neg,
              Negation = ifelse(Negation == 'INDEFINITE', 'Indefinite', Negation))
```
...
ggsave('../figures/negation_type.pdf', neg_p, width = 4.5, height = 5)
ggsave('../figures/negation_type.png', neg_p, width = 4.5, height = 5)
```
...
ggsave('../figures/CLU_type.pdf', CLU_p, width = 6.5, height = 5)
ggsave('../figures/CLU_type.png', CLU_p, width = 6.5, height = 5)
```
...
ggsave('../figures/negation_clause_type_both.pdf', fig1, width = 12, height = 5)
ggsave('../figures/negation_clause_type_both.png', fig1, width = 12, height = 5)
```
...
ggsave('../figures/function.pdf', function_p, width = 6.5, height = 5)
ggsave('../figures/function.png', function_p, width = 6.5, height = 5)
```
...
```{r}
neg %>%
  mutate(N_manual_value = ifelse(N_manual_value == 'Absent', 'no manual', 'manual')) %>% 
  count(N_manual_value, sort = TRUE) %>%
  mutate(Perc = percentage(n))
```
...
ggsave('../figures/manual_type.pdf', manual_p, width = 6.5, height = 5)
ggsave('../figures/manual_type.png', manual_p, width = 6.5, height = 5)
```
...
ggsave('../figures/nonmanual_type.pdf', nonmanual_p, width = 6.5, height = 5)
ggsave('../figures/nonmanual_type.png', nonmanual_p, width = 6.5, height = 5)
```
...
CLU_subtype_p <- grid.arrange(manual_p, nonmanual_p,
                              layout_matrix = CLU_matrix)

# Save:

ggsave('../figures/manual_vs_nonmanual.pdf', CLU_subtype_p, width = 12, height = 5)
ggsave('../figures/manual_vs_nonmanual.png', CLU_subtype_p, width = 12, height = 5)
```
...
ggsave('../figures/mouthing_negation.pdf', mouthing_p, width = 6.5, height = 5)
ggsave('../figures/mouthing_negation.png', mouthing_p, width = 6.5, height = 5)
```
...
ggsave('../figures/mouth_gesture.pdf', Mgesture_p, width = 6.5, height = 5)
ggsave('../figures/mouth_gesture.png', Mgesture_p, width = 6.5, height = 5)
```
...
ggsave('../figures/mouth_both.pdf', mouthin_mouth_gesture, width = 10, height = 5)
ggsave('../figures/mouth_both.png', mouthin_mouth_gesture, width = 10, height = 5)
```
...
HS_p <- neg %>%
  ...
  mutate(HS_new = ifelse(HS_new == 'HS',
                         'Head shake', HS_new), # renamings for plot
...
HS_p <- HS_p +
  ...
  scale_y_continuous(breaks = seq(0, 1.0, 0.2), limits = c(0, 1.05)) +
  xlab('') +
  ggtitle('(b) Type of head shake (detailed)') +
  theme_BSL()
...
ggsave('../figures/headshake_detailed.pdf', HS_p, width = 6.5, height = 5)
ggsave('../figures/headshake_detailed.png', HS_p, width = 6.5, height = 5)
```
...
HS_YesNo_p <- HS_YesNo_p +
  ...
  scale_y_continuous(breaks = seq(0, 1.0, 0.2), limits = c(0, 1.05)) +
  xlab('') +
  ggtitle('(a) Presence of head shake') +
  theme_BSL()
...
ggsave('../figures/headshake_YesNo.pdf', HS_YesNo_p, width = 6.5, height = 5)
ggsave('../figures/headshake_YesNo.png', HS_YesNo_p, width = 6.5, height = 5)
```
...
ggsave('../figures/both_headshakes.pdf', fig3, width = 12, height = 5)
ggsave('../figures/both_headshakes.png', fig3, width = 12, height = 5)
```
...
ggsave('../figures/strategy_count.pdf', strat_p, width = 6.5, height = 5)
ggsave('../figures/strategy_count.png', strat_p, width = 6.5, height = 5)
```

## Comparison to patterns for complex clauses

This section looks at the extent to which the patterns for complex clauses are different. This will be done by means of Chi-square tests. We will collate compl and neg into "both", with an identifier variable:

```{r}
both <- bind_rows(neg, compl)
both$ClauseType <- c(rep('Single', nrow(neg)),
                     rep('Non-single', nrow(compl)))
```
...
```{r}
compl %>%
  mutate(NoCLU = ifelse(is.na(CLUwithinCLU_value), 'not defined', 'some type of CLU within')) %>% 
  count(NoCLU) %>% 
  mutate(Perc = percentage(n))
```
...
```{r}
table(compl$CLUcomposite_value, compl$CLUwithinCLU_value)
```
...
ggsave('../figures/CLU_within.pdf', within_p, width = 4.5, height = 5)
ggsave('../figures/CLU_within.png', within_p, width = 4.5, height = 5)
```
...
```{r}
table(compl$CLUcomposite_value, compl$CLUcomplex_value)
```
...
ggsave('../figures/CLU_complex.pdf', within_p, width = 4.5, height = 5)
ggsave('../figures/CLU_complex.png', within_p, width = 4.5, height = 5)
```
...
ggsave('../figures/CLU_subtype.pdf', CLU_subtype_p, width = 12, height = 5)
ggsave('../figures/CLU_subtype.png', CLU_subtype_p, width = 12, height = 5)
```
