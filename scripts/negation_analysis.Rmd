...
neg_p <- neg_p +
  ...
  geom_label(aes(label = N), nudge_y = 0.055, fill = 'white') +
  geom_label(aes(label = Perc), nudge_y = -0.01, fill = 'white')
  # geom_label(aes(label = both), nudge_y = 0.01, fill = 'white') # reviewer suggestion
...
function_p <- function_p +
  geom_hline(yintercept = seq(0.2, 0.8, 0.2), color = 'grey80') +
  geom_bar(stat = 'identity', col = 'black', width = 0.6) +
  geom_label(aes(label = N), nudge_y = 0.035,
             fill = 'white', size = 4, hjust = 0.5)
...
** Bodo Winter July 10, 2025 **: Reviewer remarked on label clipping for the lower category (no non-manual negation) as it gets too close to the bottom y-axis. Fixing this requires setting different nudges for the two categories, which I'll do by setting a separate nudge variable.
...
nonmanual_p <- neg %>%
  ...
  mutate(N_nonmanual_value = ifelse(N_nonmanual_value == 'Present',
         ...
         Perc = percentage(n),
         N = str_c('N = ', n),
         label_nudges = c(-0.01, 0.013),
         text_nudges = c(0.055, 0.055 + 0.013)) %>% 
...
nonmanual_p <- nonmanual_p +
  geom_bar(stat = 'identity', col = 'black', width = 0.6) +
  geom_text(aes(label = N, y = Proportion + text_nudges)) +
  geom_label(aes(y = Proportion + label_nudges, label = Perc), fill = 'white')
...
**Addition July 10, 2025**: Reviewer 2 remarked on the floating bars, which we'll fix now. For that, as above, we'll also have to create an individual nudge variable.
...
HS_p <- neg %>%
  ...
  mutate(HS_new = ifelse(HS_new == 'HS',
         ...
         HS_new = ifelse(HS_new == 'NHS',
                         'No HS', HS_new),
         label_nudges = rev(c(0, -0.01, -0.01, -0.01, -0.01, -0.01)),
         text_nudges = rev(c(0.055 + 0.01, 0.055, 0.055, 0.055, 0.055, 0.055))) %>% 
...
HS_p <- HS_p +
  geom_bar(stat = 'identity', col = 'black', width = 0.6) +
  geom_text(aes(label = N, y = Proportion + text_nudges)) +
  geom_label(aes(label = Perc, y = Proportion + label_nudges), fill = 'white')
...
** Bodo Winter, Update July 10, 2025:** Need to fix y-axis label here too and then adjust nudges for labels.
...
strat_p <- neg %>%
  ...
  mutate(FormSum = ifelse(FormSum == 1, '1 strategy', FormSum), # renamings for plot
         ...
         nudge_labels = c(-0.01, -0.01, -0.01, 0.01),
         nudge_text = c(0.055, 0.055, 0.055, 0.055 + 0.02)) %>% 
...
strat_p <- strat_p +
  geom_bar(stat = 'identity', col = 'black', width = 0.6) +
  geom_text(aes(label = N, y = Proportion + nudge_text)) +
  geom_label(aes(label = Perc, y = Proportion + nudge_labels), fill = 'white')
