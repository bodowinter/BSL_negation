...
```{r, message = FALSE, warning = FALSE}
# Original data with Adam's coding for regular/irregular:

neg <- read_csv('../data/BSL_negation_12_June_2020.csv')

# New data with regular/irregular distinction:

neg_reg <- read_csv('../data/BSL_regularity_addition.csv')
```
...
**Addition 21/10/22:** The new table with Adam's coding for regular/irregular negation *should* be organized the same way, but better be safe than sorry and double check. Let's use `bind_cols()` to merge and then check that the identifiers are the same in all instances:

```{r}
neg <- bind_cols(neg, select(neg_reg, regularity))

# Check if all the same:

all(neg$`ClauseLikeUnit(CLU)-value` == neg_reg$`ClauseLikeUnit(CLU)-value`)

# Not the case:

which(neg$`ClauseLikeUnit(CLU)-value` != neg_reg$`ClauseLikeUnit(CLU)-value`)

# Only one:

neg[11, ]$`ClauseLikeUnit(CLU)-value`
neg_reg[11, ]$`ClauseLikeUnit(CLU)-value`
```

Ah ok, it's merely a naming difference, so nothing to worry about.
